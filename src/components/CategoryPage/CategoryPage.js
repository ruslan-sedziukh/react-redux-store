  ...
  async getProducts() {
    try {
      const response = await fetch('http://localhost:4000/', {
        ...
        body: JSON.stringify({
          "query": "query { category(input: {title: \"" + this.props.match.params.category + "\"}) { products { id name attributes { id name items { displayValue value id } } prices { currency { label symbol } amount } gallery } } }"
        })
      });
